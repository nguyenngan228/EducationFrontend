...
export const StudentDashBoard = () => {
  ...
  const loadRecCourse = async () => {
    ...
    try {
      ...
      let res = await authAPI().get(endpoints["rec_course"](pageRec))
      setRecCourse(res.data.results)
  ...
  return (
    <div className="layout">
      ...
      <h1>Recommend by AI</h1>
      {loading ? (
        <div className="text-center mt-10">
          <Spinner animation="border" />
        </div>
      ) : recCourse === null || recCourse.length === 0 ? (
        ...
        <>
          ...
          <Pagination className={styles.pagination}>
            <Pagination.Prev
              onClick={() => handlePageChange(pageRec - 1)}
              disabled={pageRec === 1}
            />
